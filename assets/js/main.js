const Input = {
    ...
    updateIncomingEnemyAttacks(enemies, centerX, centerY) {
        ...
        const contactCooldown = Math.max(260, CONFIG.ENEMY?.CONTACT_COOLDOWN_MS || 620);
        if (now - (this.lastEnemyDamageAt || 0) < contactCooldown) return;
        ...
        for (let i = 0; i < enemies.length; i++) {
            ...
            const contactLimit = contactRadius + Math.max(6, (enemy.r || 0) * 0.35);
            const retaliateReady = now < (enemy.retaliateUntil || 0) && now >= (enemy.lastRetaliateAt || 0);
            const inRange = dist <= contactLimit || (retaliateReady && dist <= contactLimit * 1.8);
            if (!inRange) continue;
            const baseDamage = Math.max(1, Number(enemy.damage) || Number(enemy.rankData?.damage) || 1);
            const ratio = dist <= contactLimit ? 1 : 0.66;
            damageTaken += baseDamage * ratio * (enemy.isElite ? 1.4 : 1);
            ailmentChance = Math.max(ailmentChance, enemy.isElite ? 0.35 : 0.22);
            enemy.lastRetaliateAt = now + 850;
            if (damageTaken > this.maxHp * 0.18) break;
        }

        if (damageTaken <= 0) return;
        this.lastEnemyDamageAt = now;
        this.updateHealth(-damageTaken, 'công kích của yêu thú');
